...
auto Trie::Get(std::string_view key) const -> const T * {
  ...
  if (node_ == nullptr) return nullptr;  // 别忘了特殊情况
  for (char ch : key) {
    if (node_->children_.count(ch) == 0) {
      return nullptr;
    }
    node_ = node_->children_.at(ch);
  }

  if (node_->is_value_node_ == 0) {
    return nullptr;
  }
...
auto Trie::Put(std::string_view key, T value) const -> Trie {
  ...
  auto new_root_ = (root_ == nullptr) ? std::make_shared<TrieNode>() : std::shared_ptr<TrieNode>(root_->Clone());
  ...
  for (size_t i = 0; i < key.size() - 1; ++i) {
    ...
    if (node_->children_.count(ch) == 0) {
      new_node_ = std::make_shared<TrieNode>();
    } else {
      new_node_ = std::shared_ptr<TrieNode>(node_->children_[ch]->Clone());
    }
  ...
  if (node_->children_.count(key.back()) == 0) {
    new_node_ = std::make_shared<TrieNodeWithValue<T>>(std::make_shared<T>(std::move(value)));
  } else {
    new_node_ = std::make_shared<TrieNodeWithValue<T>>(node_->children_[key.back()]->children_,
...
auto Trie::PostOrder(std::vector<std::shared_ptr<TrieNode>> &node_vec, std::string_view key, size_t idx) const
    -> std::shared_ptr<TrieNode> {
  if (idx >= key.size()) return nullptr;
  if (node_vec[idx] == nullptr) return nullptr;
  if (node_vec[idx]->is_value_node_ == false && node_vec[idx]->children_.empty())
    return nullptr;
  else if (node_vec[idx]->is_value_node_ == true)
    return node_vec[idx];
...
auto Trie::Remove(std::string_view key) const -> Trie {
  ...
  for (size_t i = 0; i < key.size() - 1; ++i) {
    // 1.不存在
    if (node_->children_.count(key[i]) == 0) {
      return *this;
    }
    // 2.存在的话直接换新
    else {
  ...
  for (int i = node_vec.size() - 1; i >= 0; --i) {
    if (!(node_vec[i]->is_value_node_) && (node_vec[i]->children_.empty())) {
      if (i == 0) {
        return Trie();
      } else {
        node_vec[i - 1]->children_.erase(key[i - 1]);
      }
    } else
      break;
  }
